...
//
// The title deliberately is *not* bounded. It only reaches the store when the
// user chooses "only this window", so no application can push one in on its
// own — and a title is matched with an anchored pattern, so a shortened one
// matches nothing at all. Truncating it produced exactly that: a rule saved,
// written, applied, and silently matching no window ever. Refusing to store the
// rule would have been honest; storing a rule that cannot work is not.
//
...
function parseStore(text) {
  ...
  // ceiling, counted in bytes rather than in characters. This stays because a
  // library that only holds when its caller holds is not a library that holds:
  // parseStore is what the tests run against, and what the next caller will
  // reach for.
