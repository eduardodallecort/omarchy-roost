...
// The compositor's answer to "did that config load" is the fourth thing that
// arrives from outside, and the last one that had no ceiling. It is normally
// empty and at worst a handful of lines, but it is `hyprctl` output collected
// into the shell process like any other, and a config generating megabytes of
// complaints would deliver them here.
var MAX_ERRORS_BYTES = 64 * 1024

// A ceiling on rules as well as on bytes, because the two are not the same
// limit. A megabyte of store is a megabyte Roost reads once; the rules inside
// it become window rules Hyprland evaluates against every window for as long
// as the session lasts, and the file they are written to is generated without
// a size of its own. Five hundred is two orders of magnitude past anyone's
// desktop and still nothing Hyprland would notice.
var MAX_RULES = 500

// A ceiling on the one string that arrives from a *window* rather than from a
// file. A class is whatever the application says it is, and nothing stops one
// from saying a megabyte: the rule built from it goes into the store, the store
// is then too large to read, and every rule the user has is unreadable from
// then on. Any application could do that to Roost by choosing its own class,
// without the user doing anything, so the class is bounded and a window past
// the bound is not offered.
//
// The title is *not* cut to fit. It only reaches the store when the user
// chooses "only this window", so no application can push one in on its own —
// and it is matched with an anchored pattern, so a shortened title matches no
// window that has ever existed. A rule saved, written, applied and silently
// matching nothing is worse than no rule; where a title is too long to keep,
// the save is refused instead. That refusal is the write ceiling in
// Service._persist, not a length of its own, because what cannot be stored is
// a *file* too large rather than a title too long.
//
// Two other limits touch a title without bounding it: MAX_SHOWN_TITLE cuts the
// copy that is drawn, and `shorten` is where that happens.
//
// Longest real class on a full Omarchy desktop is under fifty characters.
var MAX_MATCH_LENGTH = 512

// How much of a title the panel puts on screen. Not a ceiling on the title —
// the stored one stays whole so the rule still matches — a ceiling on the
// layout. Nothing bounds a title but the capture's own four megabytes, and Qt
// spent most of a second wrapping one that size, in the process that also
// draws the bar and the lock screen. Enough here to tell one page from
// another; the rule below it says what is actually being matched on.
var MAX_SHOWN_TITLE = 200
...
function normalizeWindow(client, monitors) {
  ...
  return {
    ...
    // rule that would reshape the whole desktop. One whose class runs to
    // hundreds of characters cannot be matched either — see MAX_MATCH_LENGTH —
    // and the picker leaves both of them out rather than offering a window it
    // would then refuse.
    matchable: cls !== UNMATCHABLE && cls.length <= MAX_MATCH_LENGTH
  }
}
...
// Text stripped of the two characters markup is built from.
//
// For strings that go to a component Roost does not own — a shell `Button`
// label, a `ButtonGroup` option. Those render through their own `Text`, which
// is `Text.AutoText` like every QML default: a string that looks like markup
// is parsed as markup, and an `<img>` inside one fetches its source over the
// network. Roost renders its own text as `Text.PlainText` and so keeps every
// character, but it cannot set that property from outside a component.
//
// So it takes the characters away instead. Nothing can open a tag without `<`
// and nothing can open an entity without `&`, which leaves a string a rich
// text parser has no construct to find, whatever heuristic decides to run it.
// That is a property of the syntax rather than of a Qt version.
//
// Only window classes and workspace names arrive here, both of them short,
// both spelled out in full elsewhere in a `Text` Roost does own.
function inertText(value) {
  return String(value == null ? "" : value).replace(/[<&]/g, "")
}

// Fill %1, %2 … in one pass, so a value cannot be read as a marker.
//
// `String.arg` substitutes one marker per call and rescans what it produced,
// so chaining it hands the second call whatever the first one inserted. A
// window class is chosen by the application: one containing `%2` — a Chromium
// web app carries percent-encoded fragments of a URL — puts the title inside
// the class. Measured: class `chrome-ex.com__a%2Fb-Default` with title
// `MY_TITLE` renders as `class chrome-ex.com__aMY_TITLEFb-Default`, on the one
// line whose job is to let somebody check what the rule matches.
//
// A single `replace` with a function never rescans its own output, so a marker
// inside a value stays a literal. QML's `String` has no two-argument `arg`, so
// this is the way to substitute more than one untrusted value into a sentence
// without breaking the sentence up for translation.
function fillOnce(template, values) {
  var list = Array.isArray(values) ? values : []
  return String(template).replace(/%(\d)/g, function (marker, digit) {
    var value = list[parseInt(digit, 10) - 1]
    return value === undefined ? marker : String(value)
  })
}

// How many bytes a string becomes on disk.
//
// The ceilings are byte counts — `stat` measures a file, and the reader caps
// with `head -c` — while a JavaScript length is UTF-16 code units. For a class
// out of Latin-1 the two agree, which is exactly why counting characters and
// calling them bytes survives every test written against a Western desktop: a
// title in Japanese is three bytes a character, so a store a third of the
// ceiling by length is over it on disk.
function utf8Length(value) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if (code < 0x80) bytes += 1
      ...
      // A surrogate pair is one character in four bytes; a lone surrogate is
      // not a character at all and is written as the replacement, in three.
      bytes += 4
      i++
    } else bytes += 3
  }
...
// A class or a title cut down to what is worth putting on screen.
//
// Two different reasons, both of them about a string Roost did not choose.
...
// Row that simply grows. A title has no ceiling at all beyond the capture's,
// so it can arrive megabytes long — laying one out took Qt most of a second
// per relayout, in the process that also draws the bar and the lock screen.
//
// Only the displayed copy is cut. The stored title is kept whole, because a
// rule is matched on the whole of it and a shortened one matches nothing.
function shorten(value, limit) {
  ...
  var cut = max - 1
  // Not through the middle of a character. An emoji is two code units, and
  // cutting between them leaves a lone surrogate, which draws as the
  // replacement box — a title ending in one reads as though Roost mangled it.
...
function buildRule(win, selection, options) {
  ...
  // A class this long is not a class. See MAX_MATCH_LENGTH.
  if (String(win["class"]).length > MAX_MATCH_LENGTH) return null
  var opts = options || {}
...
function parseStore(text) {
  ...
  //
  // The reader ahead of this one already refuses anything past the same
  // ceiling. This stays because a library that only holds when its caller
  // holds is not a library that holds: parseStore is what the tests run
  // against, and what the next caller will reach for. Counted the way the
  // reader counts — bytes, not code units — so the two cannot disagree about
  // a store that is Latin-1 to a JavaScript length and larger on disk.
  if (utf8Length(raw) > MAX_STORE_BYTES) {
  ...
  // Too many rules is refused the same way too many bytes is, and for the same
  // reason: reading the first five hundred of them would leave the rest to be
  // deleted by the next save, silently.
...
function sanitizeRule(candidate) {
  ...
  // Dropped rather than kept and truncated: a shortened class matches a
  // different set of windows than the one the rule was made for.
  if (cls.length > MAX_MATCH_LENGTH) return null
